...
func TestUploadPostSuccess(t *testing.T) {
	...
	mockFilesystemManager.EXPECT().CheckoutDirectory(uint(10)).Return(mockFilesystem)
...
func TestGetMainProjectSuccess(t *testing.T) {
	...
	mockFilesystemManager.EXPECT().LockDirectory(postID).Return(lock, nil)
	mockFilesystemManager.EXPECT().CheckoutDirectory(uint(10)).Return(mockFilesystem)
	mockFilesystem.EXPECT().CheckoutBranch("master").Return(nil)
...
func TestFilterAllPosts(t *testing.T) {
	...
	// Function under test
	fetchedPostIDs, err := postService.Filter(page, size)
	if err != nil {
		t.Fatal(err)
	}
